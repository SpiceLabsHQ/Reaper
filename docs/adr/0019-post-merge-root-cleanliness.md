...
- **ADR-0020: Rebase-First Commit Strategy with Tree-Depth Heuristic** -- Extends the merge operations that the pre- and postcondition assertions defined here must guard. The rebase and squash paths introduced by ADR-0020 all execute inside the ADR-0014 integration worktree and must satisfy the same root-cleanliness invariant verified by this ADR's assertions.
