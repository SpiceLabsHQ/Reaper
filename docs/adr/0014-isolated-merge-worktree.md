...
- **ADR-0020: Rebase-First Commit Strategy with Tree-Depth Heuristic** -- Extends this ADR by defining the specific git operations performed inside the integration worktree established here. Replaces the unconditional `--no-ff` merge with a three-path strategy (rebase + ff-only, rebase + squash, or --no-ff per sub-parent group) selected by commit count and issue tree shape.
