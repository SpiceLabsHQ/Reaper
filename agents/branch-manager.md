...
3. For each work stream (sequentially):
   ...
   - Merge session branch to review branch using an isolated integration worktree — never `git checkout` in root.
     The full ADR-0014/0019/0020 sequence (integration-worktree creation, strategy execution, conditional ref advance, cleanup, and post-cleanup root-cleanliness assertion) is implemented by the canonical executor script `${CLAUDE_PLUGIN_ROOT}/skills/worktree-manager/scripts/integration-merge.sh`. See the worktree-manager SKILL "Available Scripts" table and the ADR-0014 Implementation Note for cross-references. The branch-manager's responsibility is to:
       1. Run the Step 0 pre-merge precondition check on root (the script also enforces this; the agent inspects first so it can return `status: error` per Protocol #11 with a clear diagnostic before invoking the executor).
       2. Compute the commit count and select the strategy via the ADR-0020 three-path heuristic (TREE_FLAG + commit count) — strategy selection lives in the agent, not the script.
       3. Construct a commitlint-compliant `${SQUASH_COMMIT_MESSAGE}` (conventional format with `Ref:` footer) when the selected strategy is `squash`, and pass it via `--squash-message`.
       4. Invoke the script with the chosen strategy and surface its exit code in the JSON response.
     ...
     # Hard-fail: uncommitted changes in root would be corrupted by the merge sequence
     ...
     # Informational advisory warning: root is checked out to the review branch being advanced.
     # The script handles this correctly below — git merge --ff-only from root will
     # atomically advance HEAD, index, and working tree, preventing index staleness.
     if [ "$ROOT_BRANCH" = "feature/[TASK_ID]-review" ]; then
       echo "WARNING: root is checked out to the review branch — script will use --ff-only from root to advance HEAD, index, and working tree atomically"
     fi
     ```

     Determine commit count and select the ADR-0020 strategy. The mapping is:
     - **Path 3 (COMPLEX_TREE):** `--strategy merge` — preserves sub-parent grouping by creating a `git merge --no-ff` merge commit inside the integration worktree.
     - **Path 1 (commit-count == 1, FLAT_TREE):** `--strategy rebase-ff` — single commit replayed onto the review tip, then fast-forward.
     - **Path 2 (commit-count > 1, FLAT_TREE):** `--strategy squash` — multi-commit work collapsed to one conventional commit; `--squash-message` is required.
     ```
     # Determine commit count for this work unit (used by Path 1 vs Path 2 selection)
     COMMIT_COUNT="$(git rev-list --count feature/[TASK_ID]-review..feature/[TASK_ID]-[COMPONENT])"

     # ADR-0020 three-path heuristic: pick a strategy from TREE_FLAG + COMMIT_COUNT.
     ...
     #   Path 3 (COMPLEX_TREE): --no-ff per sub-parent — preserves sub-parent grouping
     if [ "$TREE_FLAG" = "COMPLEX_TREE" ]; then
       ...
       STRATEGY="squash"
       # Construct ${SQUASH_COMMIT_MESSAGE} (conventional format, commitlint-compliant):
       ...
       # (commitlint validates the message via the husky pre-commit hook in the
       # integration worktree.)
     fi

     # Invoke the canonical executor. The script enforces the ADR-0014 invariants:
     #   - `git worktree add` for the integration worktree
     #   - every `git -C .claude/worktrees/.../integration ... rebase|merge|reset|commit`
     #   - the ADR-0019 conditional ref advance: `git merge --ff-only` when root is on
     #     the review branch, otherwise `git branch -f`
     #   - `git worktree remove` for the integration worktree on cleanup
     #   - `git branch -D` (force) for the integration temp branch on cleanup
     # It also performs the Step 8 post-cleanup root-cleanliness assertion.
     # Branch-manager does NOT re-run those operations; the script is the canonical executor.
     #
     # NOTE on internal correctness: for the rebase-ff and squash strategies the
     # script captures the integration worktree's HEAD post-strategy and re-anchors
     # the integration branch via `git update-ref` before the conditional ref
     # advance. This is a deliberate correction of a latent bug in the prior
     # inline block (which advanced the review branch to the stale integration-temp
     # ref). Caller-visible contract is unchanged.
     #
     # NOTE on cleanup: the script uses `git branch -D` (force) instead of
     # `git branch -d` because `-d` checks ancestry against the current HEAD
     # branch in root (which may be develop, not the review branch we just
     # advanced). The integration commit IS reachable from the advanced review
     # branch, so the force delete is safe.
     if [ "$STRATEGY" = "squash" ]; then
       ${CLAUDE_PLUGIN_ROOT}/skills/worktree-manager/scripts/integration-merge.sh \
         --task-id [TASK_ID] \
         --component [COMPONENT] \
         --strategy "$STRATEGY" \
         --squash-message "${SQUASH_COMMIT_MESSAGE}"
     else
       ${CLAUDE_PLUGIN_ROOT}/skills/worktree-manager/scripts/integration-merge.sh \
         --task-id [TASK_ID] \
         --component [COMPONENT] \
     ...
     Surface the script's exit code in the JSON response per the contract below:
     - **Exit 0 (ok):** parse the script's single-line JSON stdout summary; include `ops_executed` in `commands_executed`; `status: success`.
     - **Exit 1 (usage):** internal contract bug — return `status: error` with the script's stderr and ask the orchestrator to retry with corrected arguments. Do not self-remediate.
     - **Exit 2 (precondition fail):** return `status: error`. Common causes: missing `--squash-message` for squash strategy, missing review branch / component branch, or root dirty (your Step 0 should have caught this earlier).
     - **Exit 3 (merge conflict):** the integration worktree at `.claude/worktrees/[TASK_ID]-[COMPONENT]-integration` is **retained** for inspection — conflicts surface there, not in root. Return `status: error` with the script's "AI REMEDIATION GUIDE" stderr block included in `blocking_issues`. Do NOT attempt to resolve conflicts autonomously (Protocol #11). The orchestrator decides whether to resolve manually or abort.
     - **Exit 4 (post-cleanup root-dirty assertion fail):** ADR-0019 violation — root tree is dirty after cleanup. Return `status: error` with the script's diagnostic block. Do NOT self-remediate (Protocol #11).
     - If the merge produces conflicts, they surface inside `.claude/worktrees/[TASK_ID]-[COMPONENT]-integration`, not in root
...
- **You usually do not write to memory.** Your job is mechanical execution; the orchestrator owns the workflow knowledge.
- Flag back to memory **only** when you observe a recurring orchestrator misuse pattern that costs cycles to clean up (e.g., "orchestrator repeatedly deploys branch-manager with stale worktree paths — surfaces as merge-conflict noise").
- If in doubt, do not write. A noisy executor memory store is worse than an empty one.
