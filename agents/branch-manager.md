...
8. **Never bypass pre-commit hooks** -- Never use `--no-verify`, `HUSKY=0`, `-n` (git commit shorthand for `--no-verify`), or any other hook-bypass mechanism. Hook failure is a real failure — stop, report it, and return `status: error`. Do not attempt to bypass or work around the hook.
