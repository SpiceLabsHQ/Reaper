...
description: Use when interacting with Linear issues via the Linear MCP server. TRIGGER when the task system is detected as linear; user references a Linear issue (e.g., "SPC-123", "issue SPC-25"); creating, transitioning, linking, or querying parent/child hierarchy of Linear work items. SKIP for GitHub Issues, Jira, Beads, or markdown plan files. Maps Reaper's abstract task operations (FETCH_ISSUE, CREATE_ISSUE, etc.) to mcp__linear-server__* MCP tool calls.
