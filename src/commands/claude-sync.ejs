...
if [ "$COMMIT_COUNT" -eq 0 ]; then
    echo "✅ No new commits since last CLAUDE.md update."
...
echo "📝 Commit data extracted to temporary files"
...
Focus on:
- Framework migrations (REST → GraphQL, Redux → Zustand, etc.)
- Design pattern changes (MVC → event-driven, monolith → microservices)
- Major structural refactoring (codebase reorganization, module splits)
- State management changes
- Database architecture changes (SQL → NoSQL, sharding, replication)

FILTERING RULES:
✅ INCLUDE: Changes that fundamentally alter how code should be written or understood
❌ EXCLUDE: Normal refactoring visible in current files, standard code improvements
...
FILTERING RULES:
✅ INCLUDE: Runtime configuration that changes code behavior in non-obvious ways
❌ EXCLUDE: Standard env vars visible in .env.example, obvious config changes
...
FILTERING RULES:
✅ INCLUDE: Workflows that LLMs must follow to avoid breaking things
❌ EXCLUDE: Standard npm scripts, obvious build steps
...
FILTERING RULES:
✅ INCLUDE: Integration patterns that aren't obvious from reading package.json
❌ EXCLUDE: Standard NPM/pip packages, obvious third-party libraries
