...
Deploy all panel experts after Explore agents have returned. Each receives the forum-mode initial analysis prompt with CODEBASE CONTEXT from the Explore findings (see Subagent Prompt Templates below).
...
Used in PHASE 1 — INTAKE for broad concepts (3+ domains). Deploy multiple Explore agents in parallel, each focused on a domain area:

```bash
# Deploy multiple Explore agents in parallel for broad concepts
Task --subagent_type Explore \
  --description "[Domain A] codebase recon for squadron on [concept]" \
  --prompt "Investigate [Domain A] aspects of the codebase for a design squadron working on: [CONCEPT]

Search for:
1. [Domain A]-specific implementations, patterns, and configurations
2. Dependencies and integration points relevant to [Domain A]
3. Test coverage and patterns in [Domain A] areas

Deliver a structured report:
## Existing Code ([Domain A])
[What exists today]
...
Task --subagent_type Explore \
  --description "[Domain B] codebase recon for squadron on [concept]" \
  --prompt "Investigate [Domain B] aspects of the codebase for a design squadron working on: [CONCEPT]

Search for:
1. [Domain B]-specific implementations, patterns, and configurations
2. Dependencies and integration points relevant to [Domain B]
3. Test coverage and patterns in [Domain B] areas

Deliver a structured report:
## Existing Code ([Domain B])
[What exists today]
...
Deploy one Explore agent per major domain area identified in the concept. Compile all Explore reports into a single CODEBASE CONTEXT block before deploying domain experts.
