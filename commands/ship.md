...
1. **Session context**: If you are already operating inside a worktree (e.g., dispatched by `takeoff`, or the user has been working in a `.claude/worktrees/` directory this session), use that worktree. Check your current working context and recent file operations.
2. **CWD**: Run `pwd` — if the current directory is inside a `.claude/worktrees/` path, use it.
3. **Explicit path from arguments**: If [ARGUMENTS] contains a path (e.g., `.claude/worktrees/PROJ-123-work`), use it.
4. **Auto-detect**: List worktrees under `.claude/worktrees/`. If exactly one exists, use it. If multiple exist, list them and ask which one to ship.
...
if [ -z "$WORKTREE_PATH" ]; then
    WORKTREES=$(git worktree list --porcelain | grep "^worktree.*.claude/worktrees/" | sed 's/^worktree //')
    ...
    if [ "$WORKTREE_COUNT" -eq 1 ]; then
        ...
        echo "SHIP ABORTED: No worktrees found under .claude/worktrees/"
    fi
fi
```
