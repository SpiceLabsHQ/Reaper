...
The worktree base path is read from `.reaper.yml` at command-load time:

- **WORKTREE_BASE_PATH**: !`${CLAUDE_PLUGIN_ROOT}/scripts/config-get.sh worktrees.base_path`

The default is `.claude/worktrees`. The detection logic below uses the resolved value when scanning for worktrees; the literal `.claude/worktrees/` strings appear as illustrative examples.

Resolve the worktree using this priority order:

1. **Session context**: If you are already operating inside a worktree (e.g., dispatched by `takeoff`, or the user has been working in a `${WORKTREE_BASE_PATH}/` directory this session), use that worktree. Check your current working context and recent file operations.
2. **CWD**: Run `pwd` — if the current directory is inside a `${WORKTREE_BASE_PATH}/` path, use it.
3. **Explicit path from arguments**: If [ARGUMENTS] contains a path (e.g., `.claude/worktrees/PROJ-123-work`), use it.
4. **Auto-detect**: List worktrees under `${WORKTREE_BASE_PATH}/`. If exactly one exists, use it. If multiple exist, list them and ask which one to ship.

```bash
# WORKTREE_BASE_PATH is the value resolved above from .reaper.yml (default: .claude/worktrees)

# Check CWD first
CURRENT_DIR=$(pwd)
if echo "$CURRENT_DIR" | grep -q "/${WORKTREE_BASE_PATH}/"; then
...
if [ -z "$WORKTREE_PATH" ]; then
    WORKTREES=$(git worktree list --porcelain | grep "^worktree.*${WORKTREE_BASE_PATH}/" | sed 's/^worktree //')
    ...
    if [ "$WORKTREE_COUNT" -eq 1 ]; then
        ...
        echo "SHIP ABORTED: No worktrees found under ${WORKTREE_BASE_PATH}/"
    fi
fi
```

Extract **TARGET_BRANCH** from [ARGUMENTS] if provided. If absent, fall back to the configured default base branch:

- **DEFAULT_BASE_BRANCH**: !`${CLAUDE_PLUGIN_ROOT}/scripts/config-get.sh git.default_base_branch`

Use the resolved value as `TARGET_BRANCH` (default fallback: `develop`).
...
If nothing to commit and nothing to push, report "Already shipped" and stop. Include these actionable next steps:

```
Already shipped. Nothing to commit or push.

Next steps:
- Check for a pending PR on your remote (GitHub/GitLab) — it may already be open.
- Run /reaper:status-worktrees to see the current state of all worktrees.
```
...
```bash
# Extract from worktree path: .claude/worktrees/PROJ-123-work → PROJ-123
# Or: .claude/worktrees/reaper-a3f-oauth → reaper-a3f
# Or: .claude/worktrees/issue-456-ratelimit → #456
