...
4. **Transition to ON APPROACH**: When the coding agent completes, the work unit enters the ON APPROACH state (coding done, quality gates not yet started). Capture the agent's returned `worktreePath` -- this is the path to the ephemeral agent worktree where changes live. This is a transient state before gates begin.
