...
allowed-tools: Bash(*scripts/config-get.sh*), Bash(*scripts/detect-task-system.sh*), Bash(*skills/worktree-manager/scripts/integration-merge.sh*), Skill(reaper:issue-tracker-github), Skill(reaper:issue-tracker-beads), Skill(reaper:issue-tracker-jira), Skill(reaper:issue-tracker-linear), Skill(reaper:issue-tracker-planfile)
---
## Mission Header
...
2. **Worktree path** -- look for an explicit `.claude/worktrees/...` path. May be absent.
...
- Set WORKTREE_PATH to the user-specified path, or generate one as `${WORKTREE_BASE_PATH}/[TASK_ID]-work` using the resolved value from the Git and Worktree Config section above (default: `.claude/worktrees/[TASK_ID]-work`)
...
- **No plan file found**: Run the full workflow (dispatch `reaper:workflow-planner` as a Task subagent with `MODE: PLANNING`)
...
Otherwise, dispatch `reaper:workflow-planner` as a Task subagent with `MODE: PLANNING`, passing the task ID, the full implementation context, and any plan file research as arguments. Capture the returned `agent_id` as `WORKFLOW_PLANNER_SESSION_ID`.

If the plan file has Research but no Work Units, dispatch `reaper:workflow-planner` with `MODE: PLANNING` and include the research as additional context so it does not repeat codebase investigation.
...
After strategy is determined from any source (workflow-planner, pre-planned detection, or plan file) and the plan is written to TodoWrite, apply this universal post-selection safety check before rendering the Preflight Card.

1. Run `git status --porcelain` on the repo root
2. If the output is non-empty (uncommitted changes exist in the root) AND the current strategy is `very_small_direct`, override the strategy to `medium_single_branch`
3. Log the escalation reason to the user: "Root directory has uncommitted changes — upgrading to medium_single_branch for worktree isolation"

This check applies regardless of which source determined the strategy. If the strategy is already `medium_single_branch` or `large_multi_worktree`, no override is needed.

## Quality Gate Config Check

Quality gate commands are read from `.reaper.yml` at runtime via the config reader. The reader follows a layered resolution chain (user `.reaper.yml` → bundled `defaults.yml` → caller-supplied default → hard error) and writes a `Run /reaper:init …` notice to stderr when a key is missing.

**Resolved values (read at command-load time):**

- **TEST_COMMAND**: !`${CLAUDE_PLUGIN_ROOT}/scripts/config-get.sh test.cmd`
- **LINT_COMMAND**: !`${CLAUDE_PLUGIN_ROOT}/scripts/config-get.sh lint.cmd`
- **COVERAGE_THRESHOLD**: !`${CLAUDE_PLUGIN_ROOT}/scripts/config-get.sh coverage.threshold --default 70`

If any of `TEST_COMMAND` or `LINT_COMMAND` resolved to an empty value (the reader exited non-zero and emitted a stderr notice), surface that notice to the user as an advisory and continue. Do not block takeoff on a missing key — the user's stderr notice tells them how to fix it.

Pass the resolved values verbatim into every gate-agent deployment prompt as `TEST_COMMAND`, `LINT_COMMAND`, and `COVERAGE_THRESHOLD`. The test-runner and SME reviewer agents use these values directly — they do not re-read `.reaper.yml`.

## Git and Worktree Config

These values are also read from `.reaper.yml` at command-load time and passed verbatim into every branch-manager deployment prompt. Branch-manager does not read `.reaper.yml` directly — agent files do not support runtime substitution.
...
When constructing worktree paths in this command, use `${WORKTREE_BASE_PATH}/[TASK_ID]-work` (e.g., the resolved value substituted in place of the legacy hard-coded `.claude/worktrees/` prefix). When directing branch-manager to merge, pass `BASE_BRANCH` in the deployment prompt context so the agent targets the correct branch instead of assuming `develop`.
...
Before executing any work units, deploy reaper:branch-manager to create the **session worktree** -- a named, persistent worktree at WORKTREE_PATH that owns the feature branch and real dependencies. This is the first layer of the two-layer worktree model:

1. **Layer 1 -- Session worktree**: Created once per takeoff session at `.claude/worktrees/[TASK-ID]-work`. Holds the feature branch, installed dependencies, and serves as the merge target for agent work. Created by reaper:branch-manager.
2. **Layer 2 -- Agent worktrees**: Ephemeral copies created automatically by Claude Code when agents deploy with `isolation: "worktree"`. Agents receive a fresh worktree and symlink vendor directories from the session worktree.

After the session worktree is created, run a single dependency install in it (e.g., `npm install`, `composer install`, `pip install`, `bundle install` -- detect from project config). This is the only install for the entire session. Agent worktrees symlink these dependencies rather than reinstalling.

### Branch-Manager Session Tracking

To reduce per-unit token overhead, reuse a single branch-manager agent session across all git operations within this takeoff run. Track the session using a variable named `BRANCH_MANAGER_SESSION_ID`:

- Initialize `BRANCH_MANAGER_SESSION_ID` to `null` at the start of Strategy Execution.
- After the first branch-manager Task call succeeds, capture the `agent_id` from the Task tool response and store it as `BRANCH_MANAGER_SESSION_ID`.
- On all subsequent branch-manager operations (per-unit commits and the final merge), attempt to resume the stored session using `Task --resume BRANCH_MANAGER_SESSION_ID` rather than deploying a new agent.
- **Fallback**: If `Task --resume` fails (e.g., the session is stale due to context compaction), fall back to a fresh `reaper:branch-manager` deployment, capture the new `agent_id`, and store it as `BRANCH_MANAGER_SESSION_ID` for subsequent operations.
...
3. Deploy the specified coding agent using the deployment template below, with `isolation: "worktree"` to give the agent an ephemeral worktree copy managed by Claude Code. Include the following instructions in the agent prompt:

   **Vendor symlink instruction** (include verbatim in the agent prompt):
   ```
   VENDOR SYMLINKS: Before installing dependencies, check the session worktree at
   [WORKTREE_PATH] for existing vendor directories and symlink them into your
   worktree to avoid redundant installs. Detect and symlink any that exist:
   node_modules, vendor, .venv, .bundle, target, .gradle, Pods.
   Example: ln -s [WORKTREE_PATH]/node_modules ./node_modules
   Note: Projects may automate this via a WorktreeCreate hook — if deps are already
   present after worktree creation, skip manual symlinking.
   ```

   **No-commit instruction** (include verbatim in the agent prompt):
   ```
   GIT POLICY: Do NOT run git add, git commit, git push, or any git write operations.
   The orchestrator owns all commits. Return your worktreePath and files_modified when done.
   ```

4. **Transition to ON APPROACH**: When the coding agent completes, the work unit enters the ON APPROACH state (coding done, quality gates not yet started). Capture the agent's returned `worktreePath` -- this is the path to the ephemeral agent worktree where changes live. This is a transient state before gates begin. Verify the returned `worktreePath` is a valid directory (`test -d`) before deploying gate agents. If the path is missing or invalid, render a FAULT gauge and halt the work unit.
5. Run quality gates on the completed work (see Dynamic Gate Selection and Quality Gate Protocol below). **Gate-specific isolation rules:**
   - **test-runner (Gate 1)**: Deploy directly in the agent worktree (`worktreePath` returned by the coding agent) with no `isolation` parameter. The agent worktree contains the code changes and has access to dependencies via the symlinks established during agent setup.
   - **Code reviewers (Gate 2 SME)**: Deploy with `isolation: "worktree"` for read-only review in an auto-cleaned ephemeral worktree.
   - **security-auditor (Gate 2)**: Deploy with `isolation: "worktree"` for read-only analysis in an auto-cleaned ephemeral worktree.
...
7. **Commit and merge agent work into session branch**: After all gates pass, use reaper:branch-manager to integrate the agent's changes into the session branch only -- do not merge to develop. Direct branch-manager to:
   a. Commit all changes in the agent worktree (at the `worktreePath` returned by the coding agent) using conventional commit format (e.g., `feat: implement X`, `fix: resolve Y`).
   b. Fast-forward merge (`--ff-only`) the agent branch into the session branch (the feature branch in WORKTREE_PATH).
   c. Clean up the agent worktree after successful merge.
   If `BRANCH_MANAGER_SESSION_ID` is set, resume that session via `Task --resume BRANCH_MANAGER_SESSION_ID`; otherwise deploy a fresh reaper:branch-manager agent and store the returned `agent_id` as `BRANCH_MANAGER_SESSION_ID`. If resume fails, fall back to a fresh deployment and update `BRANCH_MANAGER_SESSION_ID`. Always pass these values in the branch-manager prompt context: `TREE_FLAG: [FLAT_TREE or COMPLEX_TREE — determined at launch]`, `BASE_BRANCH: [resolved from .reaper.yml]`, and `WORKTREE_BASE_PATH: [resolved from .reaper.yml]`.
...
- **very_small_direct**: Deploy reaper:branch-manager to create a feature branch and session worktree (e.g., `.claude/worktrees/TASK-ID-work`) with dependency installation. Then deploy a single coding agent with `isolation: "worktree"`. Quality gates still apply.
- **medium_single_branch**: Deploy reaper:branch-manager to create a single session worktree (e.g., `.claude/worktrees/TASK-ID-work`) on a feature branch with dependency installation. Coding agents deploy with `isolation: "worktree"` sequentially or in parallel. File assignments must not overlap for parallel work. Worktree cleanup happens at Completion after all units pass gates.
- **large_multi_worktree**: A session worktree holds the feature branch. Each coding agent deploys with `isolation: "worktree"` for an ephemeral copy. Deploy reaper:branch-manager to create the session worktree and merge completed agent branches. Remove each per-unit agent worktree immediately after gates pass (via branch-manager).
...
WORKTREE: .claude/worktrees/repo-a3f-oauth
...
| "merge" / "ship it" / "approved" | Use reaper:branch-manager to merge the feature branch to `BASE_BRANCH` (resolved from `.reaper.yml`, default `develop`). Resume `BRANCH_MANAGER_SESSION_ID` via `Task --resume` if available; fall back to a fresh deployment if the session is stale. Always pass `BASE_BRANCH`, `WORKTREE_BASE_PATH`, and `TREE_FLAG` in the prompt context. |
...
**Merge error handling (base-branch merge)**: After branch-manager completes the merge-to-`BASE_BRANCH` operation, inspect the JSON response before announcing success:
...
3. Detect pre-planned structure or dispatch `reaper:workflow-planner` as Task subagent with `MODE: PLANNING`
4. Write plan to TodoWrite

**Per-unit loop (repeat for EACH work unit):**

>  **5. Render TAKING OFF announcement** for the current work unit
...
>  **10. Deploy branch-manager** to commit in agent worktree, --ff-only merge to session branch, cleanup
>  **--> Check TodoWrite: pending units remain? Loop to step 5**

11. Extract learning patterns from multi-iteration gates
12. Present completed work to user
13. Merge feature branch to develop on explicit user approval
