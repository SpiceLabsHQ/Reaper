...
- Set WORKTREE_PATH to the user-specified path, or generate one as `.claude/worktrees/[TASK_ID]-work`
...
Before announcing the preflight card, check whether your loaded context (CLAUDE.md or any imported files) contains explicit quality gate commands — specifically, a `## Quality Gates` section with `**Test command**:` and `**Lint command**:` fields.

**If quality gate commands are found in context:**
Use those commands directly when dispatching gate agents. Skip project config inference for test and lint commands.

**If quality gate commands are NOT found in context:**
Emit the following advisory warning card (non-blocking — takeoff continues regardless):

```
  ADVISORY
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Quality gate commands not configured for this project.
  Run /reaper:configure-quality-gates to set them up.
  Continuing with project config inference.
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

Then continue with project config inference as normal. This check requires no tool calls — it operates entirely on loaded context already in scope.
...
7. **Commit and merge agent work into session branch**: After all gates pass, use reaper:branch-manager to integrate the agent's changes into the session branch only -- do not merge to develop. Direct branch-manager to:
   ...
   If `BRANCH_MANAGER_SESSION_ID` is set, resume that session via `Task --resume BRANCH_MANAGER_SESSION_ID`; otherwise deploy a fresh reaper:branch-manager agent and store the returned `agent_id` as `BRANCH_MANAGER_SESSION_ID`. If resume fails, fall back to a fresh deployment and update `BRANCH_MANAGER_SESSION_ID`. Always pass `TREE_FLAG: [FLAT_TREE or COMPLEX_TREE — determined at launch]` in the branch-manager prompt context.
