...
for worktree in "${WORKTREE_LIST[@]}"; do
  ...
  # Collect branch age (relative time since last commit, e.g. "3 days ago")
  branch_age=$(git -C "$worktree" log -1 --pretty=format:"%cr" 2>/dev/null || echo "no commits")

  entry="${wt_name}|${state}|${gate_info}|${worktree}|${branch_age}"
...
for entry in "${SORTED_FLEET[@]}"; do
  ...
  branch_age=$(echo "$entry" | cut -d'|' -f5)
  gauge=$(render_gauge "$state")

  # Pad worktree name to 25 chars for alignment; include branch age
  printf "  %-25s %s  %-12s %-12s %s\n" "$wt_name" "$gauge" "$state" "$gate_info" "$branch_age"
done
...
for entry in "${SORTED_FLEET[@]}"; do
  ...
  entry_branch_age=$(echo "$entry" | cut -d'|' -f5)
  check_worktree_status "$worktree_path" "$entry_branch_age"
done
...
```
  WORKTREE FLEET
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  reaper-44-cache           ░░░░!!░░░░  FAULT        0/3 gates    2 hours ago
  reaper-42-auth            ██████░░░░  IN FLIGHT    1/3 gates    3 days ago
  reaper-46-deploy          ███░░░░░░░  TAKING OFF   0/3 gates    1 week ago
  reaper-45-payments        ████████░░  ON APPROACH  2/3 gates    5 hours ago
  reaper-43-migration       ░░░░░░░░░░  TAXIING     0/3 gates    4 days ago
  reaper-41-billing         ██████████  LANDED       3/3 gates    1 day ago
