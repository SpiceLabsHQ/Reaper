...
/**
 * Verifies that all three version sources agree. Calls process.exit(0) on
 * success and process.exit(1) with a clear report on failure.
 *
 * @param {string} [rootDir] - Project root to check. Defaults to the repo
 *   root (two levels up from this script in scripts/).
 */
function verifyVersionConsistency(rootDir) {
  ...
  console.log(`release:verify passed — all files agree on version ${version}`);
  process.exit(0);
}
